...
/** `/watch/providers/movie` — reference data, cached for the session. */
const useMovieProviders = ({ params, enabled = true }: IProps = {}) => {
  ...
  const { data = initialData, ...args } = useQuery<Types.IQuery.WatchProviders>({
    queryKey: ['configuration', 'movie-providers', request],
    queryFn: async () => {
      const { data } = await Api.MovieProviders({ params });
