...
import { Constants, Hooks, type Types } from '@/modules/movies';
...
const Filter = ({ open, value, onClose, onApply }: IProps) => {
  ...
  const submit = form.handleSubmit(values => {
    // Any filter change resets pagination — page 1 of the new result set.
  ...
  return (
    ...
    >
      <form className={classes.form} onSubmit={submit}>
        ...
        <div className={classes.field}>
          <span className={classes.label}>Release year</span>
          <Controller
            name="year"
            control={form.control}
            render={({ field }) => (
        ...
        <div className={classes.field}>
          ...
          <Controller
            ...
            control={form.control}
            render={({ field }) => (
              <Slider {...field} min={0} max={10} step={0.5} value={field.value ?? 0} tooltip={{ open: false }} marks={{ 0: '0', 5: '5', 10: '10' }} />
            )}
          />
        </div>
        ...
        <div className={classes.field}>
          ...
          <Controller
            name="voteCountGte"
            control={form.control}
            render={({ field }) => <InputNumber {...field} min={0} step={50} placeholder="Any" style={{ width: '100%' }} />}
          />
        </div>

        <div className={classes.field}>
          <span className={classes.label}>Maximum runtime (minutes)</span>
          <Controller
            name="runtimeLte"
            control={form.control}
            render={({ field }) => <InputNumber {...field} min={0} step={10} placeholder="Any" style={{ width: '100%' }} />}
          />
        </div>
